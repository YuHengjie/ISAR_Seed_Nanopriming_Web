...
# %%
st.markdown("<h1 style='text-align: center; color: black;'>Local interpretation based on the LightGBM model </h1>", unsafe_allow_html=True)

# %%
# understand the dataset
df_raw = pd.read_excel('data/dataset_LightGBM.xlsx',index_col = 0)
...
composition_index = list(le_composition.inverse_transform([0,1,2,3,4,5,6,7]))

le_morphology = LabelEncoder()
le_morphology.fit(X['Morphology'])
X['Morphology'] = le_morphology.transform(X['Morphology'])
morphology_index = list(le_morphology.inverse_transform([0,1]))
...
def user_input_features(dataset=X_raw,test_index=X_test_index):
    ...
    a1 = st.sidebar.selectbox(X.columns[0], list(le_composition.inverse_transform([0,1,2,3,4,5,6,7])))
    a2 = st.sidebar.slider(X.columns[1], X.iloc[:,1].min(), X.iloc[:,1].max(), 60.0)
    a3 = st.sidebar.selectbox(X.columns[2], list(le_morphology.inverse_transform([0,1])))
    a4 = st.sidebar.slider(X.columns[3], float(X.iloc[:,3].min()), float(X.iloc[:,3].max()), 100.0)
    a5 = st.sidebar.slider(X.columns[4], X.iloc[:,4].min(), X.iloc[:,4].max(), 400.0)
    a6 = st.sidebar.slider(X.columns[5], X.iloc[:,5].min(), X.iloc[:,5].max(), 0.0)
    a7 = st.sidebar.slider(X.columns[6], X.iloc[:,6].min(), X.iloc[:,6].max(), 100.0)
    customize_instance = [a1,a2,a3,a4,a5,a6,a7]
...
# %%
st.write('LightGBM is employed to make predictions. Train/test ratio: 3/1. Dataset split approach: stratified shuffle split.')
...
model_input[0] = composition_index.index(model_input[0])
model_input[2] = morphology_index.index(model_input[2])
model_input = pd.DataFrame([model_input], columns= X.columns)

# %%
model = joblib.load('model/lgbmodel.pkl')
...
with placeholder2.container():
    ...
    with f2:
        ...
        st.set_option('deprecation.showPyplotGlobalUse', False)
        st.pyplot()
st.write('The custom instances may be invalid since they ignore feature distribution and feature correlation.')
